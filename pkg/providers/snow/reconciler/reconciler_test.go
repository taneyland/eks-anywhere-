...
func TestSnowClusterReconcileSuccess(t *testing.T) {
	...
	datacenterConfig := snowDataCenter(cluster)

	bundle := bundle(cluster)
	...
	eksd := eksdRelease()
	machineConfigCP := snowCPMachineConfig(true)
	machineConfigWN := snowWNMachineConfig(true)

	setupContext(t)
	secret := testSecret(credsFileEnvVar)

	objs := []runtime.Object{cluster, managementCluster, datacenterConfig, bundle, machineConfigCP, machineConfigWN, eksd, secret}
...
func snowDataCenter(cluster *anywherev1.Cluster) *anywherev1.SnowDatacenterConfig {
...
func bundle(cluster *anywherev1.Cluster) *v1alpha1.Bundles {
...
func eksdRelease() *eksdv1alpha1.Release {
...
func snowCPMachineConfig(isValid bool) *anywherev1.SnowMachineConfig {
	return &anywherev1.SnowMachineConfig{
		...
		Spec: anywherev1.SnowMachineConfigSpec{
			SshKeyName: defaultAwsSshKeyName,
		},
...
func snowWNMachineConfig(isValid bool) *anywherev1.SnowMachineConfig {
	return &anywherev1.SnowMachineConfig{
		TypeMeta: metav1.TypeMeta{
			Kind:       "SnowMachineConfig",
			APIVersion: "anywhere.eks.amazonaws.com/v1alpha1",
